# Strict generated-asset compatibility workflow

Apply this reference only when the user explicitly requests per-asset generation, residual tracking, and full traceability. These constraints do not apply to ordinary native-object reconstruction or scientific presentations. All script paths below are relative to the skill root, not this reference directory. The single-slide builder and validator retain their original schema for compatibility.
