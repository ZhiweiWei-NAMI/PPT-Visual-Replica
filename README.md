...
把参考图片重建为可编辑 PowerPoint，也可根据论文、数据和教学材料制作科研汇报、课程演示与答辩。科研汇报、课程演示与答辩采用明确流程：先用 `imagegen` 生成逐页视觉稿，再参照视觉稿复刻为可编辑 PPTX。
...
先使用 $imagegen 生成每一页的视觉稿，再逐页复刻成可编辑 PPTX。
保留生成稿用于对照，渲染后核对版式、文字、公式和真实数据。
```
...
| 科研或教学演示 | 整理内容 → imagegen 生成逐页视觉稿 → 复刻为可编辑 PPTX → 渲染与证据核对 |
| 修改现有 PPT | 沿用模板和主题，检查修改页以及受共享样式影响的页面 |

根据论文、数据和教学材料新建 PPT 时，必须先使用 `imagegen` 技能生成每页视觉稿，再复刻；不能跳过生成阶段直接编码排版，也不能把整页图片放进 PPT 当作可编辑交付。复刻阶段可以采用原生图形、矢量和已有素材。科研数据、公式与引用以原始资料为准，不能使用生图结果编造或替换实验事实。
...
上图展示原有严格复刻流程。根据资料新建科研或教学 PPT 时，先通过 `imagegen` 生成逐页视觉稿，再进入复刻；复刻阶段可选择原生对象和已有素材。素材预览墙是按需检查步骤。
...
见 [LICENSE](LICENSE)。科研演示指导整合并重新编写了 scientific-slides 中的叙事、图表适配和演讲规划思路；未引入其外部生图脚本或模板；逐页视觉稿使用 imagegen 技能生成。
